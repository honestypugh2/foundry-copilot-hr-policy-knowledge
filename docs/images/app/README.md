...
| [`overview/03a-cost-lanes.png`](overview/03a-cost-lanes.png) ⭐ | **Crop of `03-benchmark-at-a-glance.png`, bottom row only.** The two cost lanes side by side, without the latency chart. Use this wherever the point is that the lanes cannot be summed. Pattern B appears in neither lane: its graded run sits at the deployed-agent boundary, so it has no front-door Credits, and no per-token cost was recorded for it. | Two separate cost charts side by side: the Copilot Studio lane measured in Copilot Credits per interaction, and the Foundry lane measured in estimated model cost in US dollars per answer. The axes use different units and are never combined. |
