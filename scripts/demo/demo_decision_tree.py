...
def act_1_pattern_a(content_q: str) -> Optional[int]:
    ...
    info("       caller with no repo-owned model in the path. Lab 2.1 Option 1 is the same")
...
def act_3b_pattern_c(locator_q: str) -> Optional[int]:
    ...
    info("  • Low latency on locator queries (no backend model call).")
    ...
    info("  • No repo-owned model cost on high-volume locator traffic.")
